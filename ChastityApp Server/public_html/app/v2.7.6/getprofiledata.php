    ...
    if ($userID1 == $userID2) {
        ...
        if ($query->rowCount() == 1) {
            foreach ($query as $row) {
                ...
                $timestampJoined = strtotime($joined);
                ...
                if ($username == "Hailey" || $username == "Blaine" || $username == "Zoe" || $username == "Chase") {
                    ...
                    if ($mainRoleSelected == 1 || $showKeyholderStatsOnProfile == 1) {
                        $query2 = $pdo->prepare("select count(*) as no_of_locks_managed, timestamp_locked as timestamp_first_keyheld from `Locks_V2` AS l, `ShareableLocks_V2` AS s, `UserIDs_V2` AS u where l.shared_id = s.share_id and s.user_id = u.user_id and u.user_id = :userID and l.fake = 0 and l.test = 0 order by timestamp_locked asc limit 1");
                    ...
                    if ($mainRoleSelected == 2 || $showLockeeStatsOnProfile == 1) {
                        ...
                        if ($noOfLocks > 0) {
                            $averageSecondsLocked = $cumulativeSecondsLocked / $noOfLocks;
                        } else {
                            $averageSecondsLocked = $cumulativeSecondsLocked;
                        }
                        ...
                        where l.user_id = :userID and 
                            ...
                            l.timestamp_locked > 1451606400 and 
                            l.unlocked = 0");
                        $query2->execute(array('userID' => $userID, 'timestampNow' => time()));
                        if ($query2->rowCount() == 1) {
                            foreach ($query2 as $row2) {
            ...
            array_push($JSON, array(
                ...
                'averageSecondsLocked' => $averageSecondsLocked,
                'banned' => $banned,
                ...
                'longestSecondsLocked' => $longestSecondsLocked,
                'mainRoleSelected' => $mainRoleSelected,
                'noOfLocks' => $noOfLocks,
                'noOfLocksCompleted' => $noOfLocksCompleted,
                'noOfLocksManaged' => $noOfLocksManaged,
                'noOfLocksManagingNow' => $noOfLocksManagingNow,
                'noOfRatings' => $noOfRatings,
                'privateProfile' => $privateProfile,
                'secondsLockedInCurrentLock' => $secondsLockedInCurrentLock,
                'showKeyholderStatsOnProfile' => $showKeyholderStatsOnProfile,
                'showLockeeStatsOnProfile' => $showLockeeStatsOnProfile,
