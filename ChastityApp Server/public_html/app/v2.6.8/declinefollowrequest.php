...
try {
    ...
    if ($userID1 == $userID2) {
        ...
        if ($query->rowCount() == 1) {
            $query = $pdo->prepare("delete from Relations where user_one_id = :userTwoID and user_two_id = :userOneID and status = 0");
            $result = $query->execute(array('userOneID' => $userOneID, 'userTwoID' => $userTwoID));
            if ($result == 1) { echo "Successfully Declined Follow Request"; }
        }
    }
    $query = null;
    $pdo = null;
